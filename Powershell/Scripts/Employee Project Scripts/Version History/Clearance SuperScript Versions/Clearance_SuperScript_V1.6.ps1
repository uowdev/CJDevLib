...
JMR 08-May-17 16:43: Added error handling for Migrations and Skype removal. Now Version 1.7
        ...
        do{           
            try{
                ...
                if($MailboxPresent -ne $null){
                    ...
                    $countmigrations = 1;                    
                    while (get-moverequest $Username | fl status -ne Completed)
                    {
   ...
   #----------------------------------------------------------------------------------AD Disable Start------------------------------------------------------------------------------------------>
        ...
        do{            
            Write-Host "---Checking with Operator Before Proceeding---" -foregroundcolor green -backgroundcolor Magenta
             [Validateset('Y','N', IgnoreCase)]$Proceed = (Read-Host "Proceed with User Clearance?")
            if($Redirect -eq 'Y'){ 
                $RedirectEmail = Read-Host "Please enter User with a dot for mail to be redirected to"
                }
                else {break loop}
