...
#Clears Console
clear
Write-Host "------------------NewStarter SCRIPT V1.7------------------" -foregroundcolor Blue -backgroundcolor yellow
Write-Host "-------------MUST BE RUN AS DESKTOP ADMIN-------------" -foregroundcolor Blue -backgroundcolor yellow
...
#----------------------------------------------------------------------------------Select User START--------------------------------------------------------------------->
$input = read-host "Please Select an operating User by number: `n 1 - James `n 2 - Lachlan `n 3 - Keane `n"
...
switch ($input){
    ...
    4 {$FullyUser = "zac.shearer@wisetechglobal.com"
       $3LetterUser = "CORP\zs"}
...
do{
    ...
    #---------------------------------------------------------------------------------Office365 License START--------------------------------------------------------------------------------------->
            ...
            $input = Read-Host "Country CODE of the user will be working in. EG: China - CN , Australia - AU, USA - US, S.Africa - ZA"
    ...
    #----------------------------------------------------------------------------------Mailbox Creation START---------------------------------------------------------------------------------->
            ...
            #Assinging Full Access to Ivka if DEV
<#
            if($isDev = 'Y'){
...
#>
    ...
    #----------------------------------------------------------------------------------Skype For Business START---------------------------------------------------------------------------------------->
            ...
            $count = 0
            $selection = 0
            [array]$pools = Get-CSPool | Where-Object {$_.Identity -like "*sfb.wtg.zone"}  -Verbose
